...
class RealtimeGroqService (GroqService):
    ...
    def search_tavily (self, query: str, num_results: int = 5) -> str:
        ...
        try:
            ...
            logger.info(
                ...
                len(formatted), time.perf_counter() - to,
            )
    ...
    def get_response(self, question: str, chat_history: Optional [List[tuple]] = None, key_start_index: int = 0) -> str:
        ...
        try:
            ...
            if formatted_results:
                ...
                logger.warning("[REALTIME] Tavily search returned no results for %s", search_query)
    ...
    def stream_response(self, question: str, chat_history: Optional[List[tuple]] = None, key_start_index: int = 0) -> Iterator[Any]:
        ...
        try:
            ...
            if formatted_results:
                logger.info("[REALTIME] Tavily returned results: %d chars", len(formatted_results))
                yield {"_activity": {"event": "search_complete", "message": f"Search complete: {num_results} results, {len(formatted_results)} chars of context"}}

            else:
                logger.warning("[REALTIME] Tavil returned no results for: %s", search_query)
